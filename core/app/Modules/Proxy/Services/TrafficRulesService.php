...
class TrafficRulesService
{
    ...
    private function protectionIntentTemplates(): array {
        return [
            ...
            'bot_shield' => [
                ...
                'mode' => 'recommended',
                'rules' => [
                    ['rule_table' => 'waf_rules', 'template_key' => 'waf_bot_user_agents', 'payload' => ['enabled' => true, 'name' => 'Block obvious scraper user agents', 'priority' => 55, 'type' => 'user_agent_contains', 'pattern' => 'python-requests', 'action' => 'block', 'description' => 'Generated by Bot Shield for common automation clients.', 'waf_group_id' => 'suspicious_bot', 'waf_severity' => 'medium', 'waf_confidence' => 'high', 'waf_safe_reason' => 'Targets a generic automation client instead of verified search bots.']],
                    ['rule_table' => 'waf_rules', 'template_key' => 'waf_fake_search_bots', 'payload' => ['enabled' => true, 'name' => 'Log suspicious search bot claims', 'priority' => 56, 'type' => 'user_agent_contains', 'pattern' => 'Googlebot', 'action' => 'log', 'description' => 'Generated by Bot Shield as a log-only fake bot signal until verified bot checks are configured.', 'waf_group_id' => 'suspicious_bot', 'waf_severity' => 'low', 'waf_confidence' => 'low', 'waf_safe_reason' => 'Logs search bot claims until reverse-DNS verification exists.']],
                ],
            ],
            'wordpress_hardening' => [
    ...
    private function wafMetadataPayload(array $in): array {
        $payload = [];
        foreach (['waf_group_id', 'waf_severity', 'waf_confidence', 'waf_safe_reason'] as $key) {
            if (array_key_exists($key, $in)) {
                $payload[$key] = $in[$key] === null ? null : (string) $in[$key];
            }
        }
