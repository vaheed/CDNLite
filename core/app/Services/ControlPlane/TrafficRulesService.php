...
class TrafficRulesService
{
    ...
    public function createWaf(string $domainId, array $in): array {
        $type = (string)($in['type'] ?? '');
        if (!in_array($type, ['path_contains', 'path_prefix', 'user_agent_contains', 'ip_cidr', 'country_is', 'method_is', 'header_contains', 'path_method_not_allowed'], true)) { throw new \InvalidArgumentException('invalid_type'); }
    ...
    public function discoverApiPaths(string $domainId): array {
        $stmt = Database::pdo()->prepare(
            "SELECT
                CASE
                    ...
                    WHEN path LIKE '/v1/%' THEN '/v1/'
                    WHEN path LIKE '/v2/%' THEN '/v2/'
                    WHEN path LIKE '/graphql%' THEN '/graphql'
                    ELSE '/' || split_part(trim(leading '/' from COALESCE(path, '')), '/', 1) || '/'
                END AS path_prefix,
                COALESCE(SUM(requests_count), 0) AS requests
             FROM usage_rollups
             WHERE domain_id=:domain_id
               AND path IS NOT NULL
               AND (path LIKE '/api/%' OR path LIKE '/v1/%' OR path LIKE '/v2/%' OR path LIKE '/graphql%')
             GROUP BY path_prefix
             ORDER BY requests DESC, path_prefix ASC
             LIMIT 10"
        );
        $stmt->execute([':domain_id' => $domainId]);
        $paths = [];
        foreach ($stmt->fetchAll() as $row) {
            $paths[] = [
                'path_prefix' => (string) ($row['path_prefix'] ?? '/api/'),
                'requests_24h' => (int) ($row['requests'] ?? 0),
            ];
        }
        if ($paths === []) {
            $paths[] = ['path_prefix' => '/api/', 'requests_24h' => 0, 'default' => true];
        }
        return [
            'domain_id' => $domainId,
            'paths' => $paths,
            'recommended_methods' => ['GET', 'POST', 'PUT', 'PATCH', 'DELETE', 'OPTIONS'],
            'recommended_header_key' => 'Authorization',
        ];
    }
    ...
    private function protectionIntentTemplates(): array {
        return [
            ...
            'protect_api' => [
                'name' => 'API Shield',
                'summary' => 'Protects API paths with scoped method restrictions, per-token limits, and obvious probe logging.',
                'risk' => 'moderate',
                'mode' => 'recommended',
                'rules' => [
                    ...
                    ['rule_table' => 'rate_limit_rules', 'template_key' => 'rate_api_authorization_header', 'payload' => ['enabled' => true, 'priority' => 31, 'path_prefix' => '/api/', 'key_type' => 'header_path', 'key_header_name' => 'Authorization', 'requests_per_minute' => 240, 'action' => 'block']],
                    ['rule_table' => 'waf_rules', 'template_key' => 'waf_api_allowed_methods', 'payload' => ['enabled' => true, 'name' => 'Block unsupported API methods', 'priority' => 32, 'type' => 'path_method_not_allowed', 'pattern' => '/api/:GET,POST,PUT,PATCH,DELETE,OPTIONS', 'action' => 'block', 'description' => 'Generated by API Shield to block methods outside the reviewed API method set.', 'waf_group_id' => 'api_method_probe', 'waf_severity' => 'medium', 'waf_confidence' => 'high', 'waf_safe_reason' => 'Only applies to /api/ paths and permits common API methods.']],
                    ['rule_table' => 'waf_rules', 'template_key' => 'waf_api_method_probe', 'payload' => ['enabled' => true, 'name' => 'Log suspicious API method probes', 'priority' => 70, 'type' => 'path_method_not_allowed', 'pattern' => '/api/:GET,POST,PUT,PATCH,DELETE,OPTIONS,HEAD', 'action' => 'log', 'description' => 'Generated by API Shield to surface unusual API method probes before blocking.', 'waf_group_id' => 'api_method_probe', 'waf_severity' => 'medium', 'waf_confidence' => 'medium', 'waf_safe_reason' => 'Runs in log mode because method probes can overlap with diagnostics.']],
                ],
            ],
            'smart_rate_limiting' => [
            ...
            'emergency_protection' => [
                ...
                'summary' => 'Temporarily challenges site-wide traffic before origin routing and blocks common incident scanner patterns.',
                'risk' => 'risky',
                'mode' => 'confirm_first',
                'rules' => [
