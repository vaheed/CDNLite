...
test('settings persist and secrets remain masked', async ({ page }) => {
  ...
  const originalApiUrl = await apiUrl.inputValue();
  await apiUrl.fill('http://pdns-auth:8081');
  ...
  await page.reload();
  await expect(page.getByLabel('Api Url')).toHaveValue('http://pdns-auth:8081');
  ...
  await expect(page.getByText('PowerDNS connection succeeded.')).toBeVisible();

  await page.getByLabel('Api Url').fill(originalApiUrl);
  await page.getByRole('button', { name: 'Save changes' }).click();
  await expect(page.getByText('Settings saved.')).toBeVisible();
});
