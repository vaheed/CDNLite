...
const domains=ref<Domain[]>([]),selected=ref<AuditEntry|null>(null),loading=ref(true),error=ref(''),fromInput=ref(''),toInput=ref('');const limit=ref(50),offset=ref(0);const filters=reactive<AuditFilters>({actor:'',action:'',resource_type:'',domain_id:''});const result=ref<PaginatedResult<AuditEntry>>({items:[],total:0,limit:limit.value,offset:0});
onMounted(async()=>{domains.value=await domainsApi.list();await load();});async function load(){loading.value=true;error.value='';try{result.value=await auditLogApi.list({...filters,limit:limit.value,offset:offset.value});}catch(e){error.value=e instanceof Error?e.message:'Could not load audit log.';}finally{loading.value=false;}}
function applyFilters(){filters.from=toEpoch(fromInput.value);filters.to=toEpoch(toInput.value);offset.value=0;void load();}function clear(){Object.assign(filters,{actor:'',action:'',resource_type:'',domain_id:'',from:undefined,to:undefined});fromInput.value='';toInput.value='';applyFilters();}function setLimit(value:number){limit.value=value;offset.value=0;void load();}function setOffset(value:number){offset.value=value;void load();}function json(value:unknown){return JSON.stringify(value??{},null,2);}function toEpoch(value:string){return value?Math.floor(new Date(value).getTime()/1000):undefined;}
