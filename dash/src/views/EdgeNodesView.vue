<template><section class="space-y-6"><div><h1 class="text-3xl font-black text-slate-950 dark:text-white">Edge Nodes</h1><p class="text-slate-600 dark:text-slate-400">Identity, heartbeat freshness, region, version, and online/offline health.</p></div><DataTable title="Edge health table" :rows="rows" :columns="columns"><template #identity="{ row }"><div class="flex items-center gap-2"><span class="font-mono">{{ row.edge_id }}</span><StatusBadge v-if="row.identity_status === 'warning'" status="warning" label="Suspicious" /></div></template><template #health="{ row }"><StatusBadge :status="String(row.health)" /></template></DataTable><ChartCard title="Online / Offline" :option="chart" /></section></template>
