<template>
  <section class="space-y-6">
    <div><h1 class="text-3xl font-black text-slate-950 dark:text-white">Domains</h1><p class="text-slate-600 dark:text-slate-400">Full lifecycle management for CDN domains, origins, and proxy state.</p></div>
    ...
    <form v-if="showForm && editingId" class="card grid gap-4 p-4 sm:p-5 xl:grid-cols-2" @submit.prevent="saveDomain">
      ...
      <label class="space-y-2">
        ...
        <p class="text-xs leading-5 text-slate-500 dark:text-slate-400">Controls whether the domain is active in generated configuration.</p>
      </label>
      ...
      <div class="xl:col-span-2 flex flex-wrap justify-end gap-2">
        ...
        <button class="button-primary" :disabled="saving">{{ editingId ? 'Save changes' : 'Create domain' }}</button>
      </div>
    </form>
...
const domainSchema = z.object({ name: z.string().min(1, 'Domain name is required.'), domain: z.string().min(1, 'Domain is required.'), origin_scheme: z.string().min(1, 'Origin scheme is required.'), origin_host: z.string().min(1, 'Origin host is required.'), origin_port: z.coerce.number().int().positive('Origin port must be a positive integer.') });
const help = {
  name: { label: 'Name', what: 'Human-readable domain name shown in the admin.', works: 'Does not affect routing; used for management.', example: 'Main Webdomain', required: true },
...
const domainRows = computed(() => domains.value.map((domain) => ({ ...domain, origin: `${domain.origin_scheme ?? 'http'}://${domain.origin_host}:${domain.origin_port}`, actions: '' })));
...
async function saveDomain() {
  ...
  if (!editingId.value && !String(form.origin_shield_secret).trim()) {
    ...
    formError.value = 'Fix the highlighted fields before saving the domain.';
    return;
  }
  saving.value = true;
  try {
    ...
    console.error('[domains-form]', error);
    formError.value = messageFor(error, editingId.value ? 'Unable to update domain.' : 'Unable to create domain.');
  }
  finally { saving.value = false; }
}
async function toggleProxy(row: Record<string, unknown>) { const id = String(row.id); if (row.proxy_enabled) await domainsApi.disableProxy(id); else await domainsApi.enableProxy(id); await load(); }
...
function editDomain(row: Record<string, unknown>) {
