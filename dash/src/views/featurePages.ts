...
export const featurePages: FeaturePage[] = [
  {
    ...
    fields: [
      { name: 'type', label: 'Type', what: 'DNS record type.', works: 'A/AAAA/CNAME/etc. define how hostnames resolve.', example: 'A', required: true },
  ...
  },
  {
    ...
    fields: [
      ...
      { name: 'status_code', label: 'Status code', what: 'HTTP redirect status.', works: '301/308 are permanent; 302/307 are temporary.', example: '308', type: 'number' },
  ...
  },
  {
    ...
    fields: [
      ...
      { name: 'cache_query_string_mode', label: 'Query string mode', what: 'Controls whether query strings affect cache keys.', works: 'ignore_all improves hit ratio but can serve wrong content if query changes responses.', example: 'include_all' },
  ...
  },
  {
    ...
    fields: [
      { name: 'type', label: 'Type', what: 'What part of the request to inspect.', works: 'Example: path_prefix checks URL path prefix.', example: 'user_agent_contains', required: true },
      { name: 'pattern', label: 'Pattern', what: 'Value to match.', works: 'Match behavior depends on selected type.', example: 'curl', required: true },
      { name: 'action', label: 'Action', what: 'What edge should do when matched.', works: 'block denies, log records, allow permits.', example: 'block', required: true },
