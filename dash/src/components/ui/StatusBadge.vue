<template>
  <span :class="classes" class="inline-flex items-center rounded-full px-2.5 py-1 text-xs font-semibold ring-1 ring-inset">{{ label ?? status }}</span>
</template>
...
const classes = computed(() => {
  ...
  if (['warning', 'stale', 'pending', 'info'].includes(status)) return 'bg-amber-50 text-amber-700 ring-amber-200 dark:bg-amber-400/10 dark:text-amber-200 dark:ring-amber-300/20';
