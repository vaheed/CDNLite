...
export interface CacheSettings { enabled: boolean; default_edge_ttl_seconds: number; default_browser_ttl_seconds: number | null; cache_query_string_mode: string; respect_origin_cache_control: boolean; cache_authorized_requests: boolean; stale_if_error_seconds: number; static_asset_cache_enabled: boolean; ignore_query_strings_for_static: boolean; bypass_logged_in_users: boolean; cache_methods: string[]; cache_status_code_policy: Record<string, boolean>; bypass_headers: string[]; bypass_cookies: string[]; vary_headers: string[]; cache_key_dimensions: Record<string, unknown>; debug_headers_enabled: boolean; stale_while_revalidate_seconds: number; negative_ttl_seconds: number; max_object_size_bytes: number; }
