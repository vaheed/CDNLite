...
describe('diagnostics', () => {
  it('classifies edge heartbeat, ssl, cache, and ops cards', () => {
    ...
    expect(buildOpsDiagnostic({ sites: [{ id: 's1', name: 'Demo', domain: 'demo.local', origin_host: 'core', origin_port: 8080, proxy_enabled: true }], edges: [], usage: { total_requests: 5 } }).sites).toBe(1);
    expect(buildOpsDiagnostic({ sites: [], edges: [], usage: { requests_count: 10, bytes_in: 21709, bytes_out: 60108, records: 10 } }).totalRequests).toBe(10);
  });
});
