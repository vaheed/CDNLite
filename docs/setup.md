...
| `CDNLITE_EDGE_CONFIG_MAX_BYTES`, `CDNLITE_EDGE_CONFIG_REFRESH_SECONDS` | Maximum accepted edge snapshot size and worker config refresh interval. Core publish rejects oversized snapshots with `config_snapshot_too_large`; edge workers use the same byte limit before loading a file. |
