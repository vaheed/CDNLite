...
`router.lua` lowercases the request host and strips any port. It loads `/var/lib/cdnlite/config.json`, looks up `hosts[host]`, chooses an upstream, and calls `proxy.forward(domain)`. Missing host or unknown host returns 502.
...
The router also evaluates a per-domain/path rate-limit rule from snapshot `rate_limits`. Counters are stored in `lua_shared_dict cdnlite_limits` as 60-second buckets. When over threshold with `action=block`, the edge returns `429` JSON `{"error":"rate_limited","request_id":"..."}` and sets `Retry-After: 60`.

## Geo Upstreams

The router checks `X-CDNLITE-Country` first, then `CF-IPCountry`. If a matching country exists in `geo_upstreams`, that upstream is used. Otherwise `DEFAULT` is used when present; otherwise the domain `upstream` is used.
