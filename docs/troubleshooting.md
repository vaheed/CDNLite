...
| Visitors repeatedly receive challenge responses | Edge clearance cookies cannot be verified, the client IP changed, or the edge fleet uses different `CDNLITE_EDGE_CLEARANCE_SECRET` values. | Set the same strong `CDNLITE_EDGE_CLEARANCE_SECRET` on every edge, keep clocks sane, and verify the client is returning the `__cdnlite_clearance` cookie. |
