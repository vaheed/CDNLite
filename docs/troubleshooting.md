...
| SSL request shows no queued scheduler work and no `_acme-challenge` record | The domain is still `pending_nameserver`, so managed SSL is rejected before ACME starts. Proxying a DNS record is not required for DNS-01 issuance. | Verify nameservers or activate the domain first, then request SSL again. |
