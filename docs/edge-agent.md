...
| `register.sh` | Signed `POST /api/v1/edge/register`. |
| `heartbeat.sh` | Signed `POST /api/v1/edge/heartbeat`. |
...
`CORE_URL`, `EDGE_ID`, `EDGE_TOKEN`, `EDGE_CONFIG_PATH`, and `METRIC_PATH` are required for normal operation. Registration also uses `EDGE_HOSTNAME`, `EDGE_PUBLIC_IP`, `EDGE_REGION`, and `EDGE_VERSION`.

## Register Flow

1. Operator provisions token: `php artisan cdn:edge:register-token --edge_id=<id> --token=<token>`.
2. Agent builds JSON body with edge identity.
3. Agent signs `POST /api/v1/edge/register`.
4. Core verifies token, timestamp, nonce, signature, and edge ID match.
5. Core upserts `edge_nodes` with status `online`.

## Heartbeat Flow

`heartbeat.sh` sends `{"edge_id":"$EDGE_ID"}`. Core updates `last_heartbeat`, `status=online`, and `updated_at`. If no edge node row exists after auth succeeds, core returns `edge_not_found`.

## Config Pull Flow

`pull_config.sh` signs a GET request with an empty body hash and writes the response to a temporary file before moving it to `EDGE_CONFIG_PATH`. The script does not pass `if_version`; it always writes the full response it receives.
